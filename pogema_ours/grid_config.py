...
from pogema_ours.utils import CommonSettings

from typing_extensions import Literal


class GridConfig(CommonSettings,):
    on_target: Literal['finish', 'nothing', 'restart'] = 'finish' #TODO:更换为任意一个达到目标就结束
    seed: Optional[int] = None
    size: int = 8
    density: float = 0.3
    num_agents: int = 2
    num_criminal: int = 1 #added
    obs_radius: int = 5
    agents_xy: Optional[list] = None
    targets_xy: Optional[list] = None
    collision_system: Literal['block_both', 'priority', 'simultaneous'] = 'priority'
    ...
    alpha: float = 0.8
    replay_directory: str = "./renders/distance/"

    @validator('alpha')
    def alpha_validation(cls, v):
        assert 0 <= v , "alpha must be in [0, +inf]"
        return v
    ...
    #added
    @validator('num_criminal')
    def class_agents_validation(cls, v, values):
        assert 1 <= v, "there must be at least one criminal"
        return v
    #added
    ...
    def playability_validate():
        """check the connectness of police, criminal, Exits in map
        """
        pass
...
class OneCriminal(GridConfig):
    density: float = 0.2 #change the density to 0.2
    collision_system: Literal['priority'] = 'priority'
    obs_radius: Literal[5] = 5
    ...
    size: Literal[16] = 16
    max_episode_steps: Literal[256] = 256
    num_agents: Literal[3] = 3
    num_criminal: Literal[1] = 1
    map_name: Literal['OneCriminal'] = 'OneCriminal'
