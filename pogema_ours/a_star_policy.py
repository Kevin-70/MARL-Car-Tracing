...
def h2(node, agents,fixed):
    if fixed is True:return 0
    nx, ny = node
    dis = None
    for n in agents:
        ax, ay = n
        dis_i = abs(ax - nx) + abs(ay - ny)
        if dis is None or dis > dis_i:
            dis = dis_i
    return -dis


def a_star(start, target, grid: GridMemory, max_steps=10000, agents=[],fixed=True):
    open_ = list()
    closed = {start: None}

    heappush(open_, Node(start, 0, h(start, target)+h2(start, agents,fixed)))
    ...
    for step in range(int(max_steps)):
        ...
        for n in [(u.i - 1, u.j), (u.i + 1, u.j), (u.i, u.j - 1), (u.i, u.j + 1)]:
            if not grid.is_obstacle(*n) and n not in closed:
                heappush(open_, Node(n, u.g + 1, h(n, target)+h2(n, agents,fixed)))
                closed[n] = (u.i, u.j)
...
class AStarAgent:
    def __init__(self, seed=0, fixed_p=True):
    ...
    def act(self, obs):
        ...
        agents = np.argwhere(agents == 1.) # get tuple of positions
        ...
        path = a_star(xy, target_xy, self._gm, agents=agents, fixed=self.fixed)
        if len(path) <= 1:
            action = 0
        else:
            (x, y), (tx, ty), *_ = path
