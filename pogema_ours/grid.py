...
from pogema_ours.generator import generate_obstacles, generate_positions_and_targets_fast, \
    get_components
from pogema_ours.grid_config import GridConfig
from pogema_ours.grid_registry import in_registry, get_grid
from pogema_ours.utils import render_grid


class Grid:
    """
    for agent_idx in range(self.grid_config.num_agents):
        if(self.grid_config.class_agents[agent_idx]==1):
            self.grid.get_targets_xy[agent_idx] = self.grid.get_agents_xy[0]
    """
    def __init__(self, grid_config: GridConfig, add_artificial_border: bool = True, num_retries=10):
        ...
        #Configuration added at initialization
        for agent_idx in range(grid_config.num_criminal, grid_config.num_agents):
            self.finishes_xy[agent_idx] = self.starts_xy[0]
        #Configuration added at initialization
    ...
    def on_goal(self, agent_id):
        # modified
        if agent_id < self.config.num_criminal:
            return self.positions_xy[agent_id] == self.finishes_xy[agent_id]
        else:
            return list(self.finishes_xy[agent_id]) in (np.array(self.config.MOVES) + self.positions_xy[agent_id]).tolist()
