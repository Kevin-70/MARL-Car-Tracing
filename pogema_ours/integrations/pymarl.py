...
from pogema_ours.envs import _make_pogema
...
class PyMarlPogema:
    ...
    def __init__(self, grid_config, mh_distance=False):
        ...
        self.env = AnimationMonitor(self.env, AnimationConfig(directory=grid_config.replay_directory, save_every_idx_episode=5000))
    ...
    def step(self, actions):
        ...
        if done:
            ...
            info.update(CriminalOnGoal=infos[0]['metrics']['CriminalOnGoal'])
            info.update(PoliceOnGoal=infos[0]['metrics']['PoliceOnGoal'])
    ...
    def get_dict_obs(self):
        return np.array(self._observations[1])

    def get_obs_agent(self, agent_id):
        return np.array(self._observations[0][agent_id]).flatten()

    def get_obs_size(self):
        return len(np.array(self._observations[0][0]).flatten())
    ...
    def reset(self):
        ...
        self._observations, _ = self.env.reset()
        return np.array(self._observations[0]).flatten()

    def save_replay(self):
        self.env.save_animation('/home/maddpg/pogema_env/renders/dqn.svg')
        self.env.save_animation("/home/maddpg/pogema_env/renders/dqn_ego.svg", AnimationConfig(egocentric_idx=0))
        return
