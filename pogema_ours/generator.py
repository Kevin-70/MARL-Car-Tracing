...
from pogema_ours import GridConfig


def generate_obstacles(grid_config: GridConfig, rnd=None):
    if rnd is None:
        rnd = np.random.default_rng(grid_config.seed)
    rnd_map = rnd.binomial(1, grid_config.density, (grid_config.size, grid_config.size))

    #connectivity 
    c2p, p2c = get_components(grid_config, rnd_map.copy(), None, None)
    start_id = max(grid_config.FREE, grid_config.OBSTACLE) + 1
    for k in list(c2p.keys()):
        if k < start_id: c2p.pop(k)
    
    while len(c2p.keys()) > 1:
        start = np.array(*rnd.choice(c2p[start_id], 1))
        finish_id = [max(c2p.keys())]
        finish = np.array(*rnd.choice(c2p[finish_id[0]], 1))
        moves = []
        if start[0] > finish[0]: moves.append([1, 0])
        if start[0] < finish[0]: moves.append([-1, 0])
        if start[1] > finish[1]: moves.append([0, 1])
        if start[1] < finish[1]: moves.append([0, -1])

        while True:
            finish += moves[rnd.integers(len(moves))]
            if len(moves) == 2 and (start == finish).any():
                if (start != finish + moves[0]).all(): moves.pop(0)
                else: moves.pop()
            if p2c[tuple(finish)] == grid_config.OBSTACLE: 
                p2c[tuple(finish)] = finish_id[0]
                rnd_map[finish[0]][finish[1]] = 0
            elif p2c[tuple(finish)] == start_id or p2c[tuple(finish)] not in c2p.keys():
                for f in finish_id: c2p.pop(f)
                break
            elif p2c[tuple(finish)] not in finish_id:
                finish_id.append(p2c[tuple(finish)])
...
#distance validation
def check_init_distance_failed(c, grid, positions_xy, finishes_xy):
    base_num = max(c.FREE, c.OBSTACLE) + 1
    q = []
    grid[finishes_xy[0]] = base_num
    q.append(finishes_xy[0])
    while len(q):
        x, y = q.pop(0)
        for dx, dy in tuple(c.MOVES):
            nx, ny = x + dx, y + dy
            if 0 <= nx < len(grid) and 0 <= ny < len(grid[0]):
                if grid[nx, ny] == c.FREE:
                    grid[nx, ny] = grid[x, y] + 1
                    q.append((nx, ny))
    d = grid[positions_xy[0]]
    for xy in positions_xy:
        if grid[xy] < d: return False
    return True
...
def generate_positions_and_targets_fast(obstacles, grid_config):
    ...
    rnd = np.random.default_rng(c.seed)
    rnd.shuffle(order)
    attempt = placing(order=order, components=components, grid=grid, start_id=start_id, num_agents=c.num_agents)

    #distance validation
    while check_init_distance_failed(c, obstacles.copy(), *attempt):
        rnd.shuffle(order)
        attempt = placing(order=order, components=components, grid=grid, start_id=start_id, num_agents=c.num_agents)
